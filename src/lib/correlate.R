correlate <- function(data, method = "spearman", ...) {
  switch(method,
    ...
    "jaccard" = correlate_jaccard(data = data, ...),
...
#' Co-occurrence tests using Jaccard metrics
#'
#' Jaccard Index ignores co-exclusion and is therefore suitable for microbiome meta studies
#' @references https://doi.org/10.1371/journal.pone.0187132
#' @param min_abundance abundance must be higher than this value to be counted as occurring
correlate_jaccard <- function(data, min_abundance_perc = 0, B = 10e3, seed = 1337, ...) {
  res <-
    data %>%
    colnames() %>%
    combn(2) %>%
  ...
  # normalize and transform to presence / absence matrix
  data <- (data / rowSums(data) > (min_abundance_perc / 100)) %>% apply(2, as.integer)
  ...
  res[["data"]] <- pbmcapply::pbmcmapply(f, res$from, res$to)
