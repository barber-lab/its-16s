...
coabundance <- function(edges, nodes = NULL, method = NULL, max_pval = 0.05, min_abs_estimate = NULL, ...) {
  ...
  if (!is.null(nodes)) {
    taxons <- edges$from %>%
      union(edges$to) %>%
      unique()
    nodes <- nodes %>% dplyr::filter(taxon %in% taxons)
  }
...
coabundance.tbl_graph <- function(graph, nodes = NULL, method = NULL, max_pval = 0.05, min_abs_estimate = NULL, ...) {
  edges <- graph %>%
    ...
    activate(edges) %>%
    as_tibble() %>%
    select(from = from_taxon, to = to_taxon)
...
tbl_graph_to_coabundance <- function(g) {
  g %>%
    get.edgelist() %>%
    data.frame() %>%
    as_tibble() %>%
    ...
    mutate(
      estimate = g %>% get.edge.attribute("estimate"),
      p.value = g %>% get.edge.attribute("p.value")
    ) 
}

as_tbl_graph.tbl_graph <- function(cor_res, ...) {
  cor_res
}

as_tbl_graph.tbl_df <- function(cor_res, nodes = NULL, method = NULL, ...) {
  if (!all(c("from", "to", "estimate") %in% colnames(cor_res))) {
    stop("cor_res must have at least columns from, to and estimate")
  }
...
#     dplyr::rename(from = from_taxon, to = to_taxon)
#
#   coabundance(cor_res = cor_res, edges = edges, nodes = nodes, method = method)
# }
